<template>
    <div>
        <!-- Titre de la liste -->
        <div>
            <h1>{{ listName }} ({{ filteredFullList.length }})
        ...
        <b-collapse id="gamesFilter" class="mt-2">
            <b-card>
                les filtres: parties, auteur, illustrateur
                <b-container fluid>
                    <b-row class="my-1">
                        <b-col sm="4">
                            <b-form-input 
...
export default {
    ...
    methods: {
        fetchData: function() {
            ...
            this.displayedGameList = this.gameList.slice(0, displayLength);
            this.search();
        },
        ...
        infiniteHandler: function($state) {
            if (this.displayedFilteredGameList.length !== this.filteredFullList.length) {
                this.search(this.gameList);
                this.displayedFilteredGameList = this.filteredFullList.slice(0, this.displayedFilteredGameList.length + displayLength);
                $state.loaded();
            }
            else {
                $state.complete();
        ...
        search: function() {
           let filteredList = this.gameList;
            ...
            if(this.searchIllustrateur !== '') {
                filteredList = this.searchIllustrator(filteredList, this.searchIllustrateur);
            }
            if(this.searchTitre !== '') {
            ...
            this.displayedFilteredGameList = this.filteredFullList.slice(0, this.displayedFilteredGameList.length);
            if(this.$refs.infiniteLoading){
                this.$refs.infiniteLoading.stateChanger.reset(); 
            }
        ...
        searchDesigner: function(gameListToFilter, auteur) {
            return gameListToFilter.filter(game => {
                return game.authors.join(', ').toLowerCase().includes(auteur);
            });
        },
        searchIllustrator: function(gameListToFilter, illustrateur) {
            return gameListToFilter.filter(game => {
                return game.artists.join(', ').toLowerCase().includes(illustrateur);
            });
        },
